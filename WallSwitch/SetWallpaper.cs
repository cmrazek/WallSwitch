...
namespace WallSwitch
{
	class SetWallpaper
	{
		...
		private void SetWin(string fileName)
		{
			...
			{
				Log.Write(ex, "Failed to set registry keys.");
				key.Close();
			}
		...
		public void Set(Theme theme, string[] files)
		{
			...
			foreach (Screen screen in Screen.AllScreens)
			{
				...
				if (rect.Left + rect.Width > fullSize.Width) fullSize.Width = rect.Left + rect.Width;
				if (rect.Top + rect.Height > fullSize.Height) fullSize.Height = rect.Top + rect.Height;
			}
		...
		private void ClearBackground(Screen screen, int opacity)
		{
			...
			{
				// Gradient
				brush = new LinearGradientBrush(screen.Bounds, topColor, bottomColor, LinearGradientMode.Vertical);
			}

			_g.FillRectangle(brush, screen.Bounds);
			brush.Dispose();
		}
